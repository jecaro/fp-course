...
hex ::
  Parser Char
hex =
  let convert :: List Char -> Parser Char
      convert cs =
        case readHex cs of
          Empty -> constantParser $ UnexpectedString cs
          Full i -> pure (chr i)
   in convert =<< replicateA 4 (satisfy isHexDigit)
...
eof ::
  Parser ()
eof =
  let f :: Input -> ParseResult ()
      f Nil = Result Nil ()
      f (c :. _) = UnexpectedChar c
   in P f
